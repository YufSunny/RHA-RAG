...
基于 [LangGraph](https://langchain-ai.github.io/langgraph/)、[Milvus Lite](https://milvus.io/docs) 与 [DeepSeek V4](https://api.deepseek.com) 构建。
...
激活后在其中执行 `pip install -r requirements.txt`。
...
- **按浏览器会话隔离。** 会话 id 存于 `localStorage`（各标签页独立，刷新不丢失）。历史保存在服务器内存中，重启后清空。
...
- **清空对话。** 点击 “Clear chat” 按钮（或 `POST /api/clear`）可清空该会话的历史。
...
| 对话记忆 | 按会话的问答历史（管理可调 `MAX_HISTORY_TURNS`） |
...
├── config.py              管理可调设置（如 MAX_HISTORY_TURNS）
├── rha_rag/                核心包
...
测试会索引 `test/` 中的 6 篇文档语料（覆盖所有支持的文件类型），针对 `test/questions.txt` 中的问题进行检索，断言命中正确来源，再运行完整 7 节点图谱并断言生成非空的带引用答案。
